...
function HomeContent() {
  ...
  const handleLoadSession = useCallback(
    async (session: Session, checkCurrent: boolean) => {
      setIsLoadingSession(true);
      try {
        // If checkCurrent is true and there's unsaved content, save current session first
        if (checkCurrent && sections.some((s) => s.content.trim())) {
          const shouldSave = confirm(
            "Current page has unsaved content. Save it as a session before loading the new one?",
          );
          if (shouldSave) {
        ...
        setSections(JSON.parse(JSON.stringify(session.sections))); // Deep copy
        setUseTranslated(session.useTranslated);
        toast.success(`Session loaded: ${session.id}`);
      } catch (error) {
    ...
    [sections, useTranslated, saveCurrentSession],
  );
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-blue-50 to-indigo-100">
      ...
      {/* Main Content Area - offset by collapsed sidebar width on large screens */}
      <div className="flex-1 min-w-0 transition-all duration-300 lg:ml-16">
        {/* Header */}
              ...
              <div className="lg:sticky lg:top-8 space-y-4">
                ...
                <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <ul className="text-sm text-blue-800 space-y-1">
                    ...
                    <li>
                      • Toggle between original and translated content in
                      preview
                    </li>
                    <li>
...
// Wrap with TranslationProvider and SessionProvider
